    ...
    describe('More complex example', () => {
        ...
        it('should handle a successful flow', async () => {
            ...
            async function getUserAsync(
                _userId: string,
            ): Promise<Result<User, NetworkError | HttpResponseError>> {
                return Promise.resolve(
                    succeed<User>({ name: 'UserName', documentIds: ['1', '2', '3'] }),
                );
            }

            const getDocumentsAsync = jest.fn((_documentIds: string[]) =>
                Promise.resolve(
                    succeed<Document[]>([
            ...
            const checkDocumentHasBeenValidated = jest.fn(
                (document: Document) => document.status === 'VALIDATED',
            );
            ...
            expect(logValidDocuments).toHaveBeenCalledWith([
                { name: 'Document 1', status: 'VALIDATED' },
                { name: 'Document 3', status: 'VALIDATED' },
            ]);
        ...
        it('should handle an error', async () => {
            ...
            const getDocumentsAsync = jest.fn((_documentIds: string[]) =>
                Promise.resolve(
                    defect<HttpResponseError, Document[]>({
                        [ErrorTag]: 'HttpResponseError',
                        status: 500,
                    }),
                ),
